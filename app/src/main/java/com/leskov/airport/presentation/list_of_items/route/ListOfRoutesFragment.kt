...
import com.leskov.airport.databinding.FragmentListOfRoutesBinding
import com.leskov.airport.domain.entity.TypeOfEntity
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class ListOfRoutesFragment : BaseVMFragment<ListOfRoutesViewModel, FragmentListOfRoutesBinding>() {

    override val layoutId: Int
        get() = R.layout.fragment_list_of_routes

    override val viewModel: ListOfRoutesViewModel by viewModels()

    private val adapter = ListOfRoutesAdapter()
    ...
    override fun initListeners() {
        ...
        binding.flAddItem.setOnClickWithDebounce {
            navController.navigate(
                R.id.action_listOfRoutesFragment_to_insertNewItemFragment,
                bundleOf("type" to TypeOfEntity.ROUTE)
            )
        }

        adapter.setOnItemClickListener {
            navController.navigate(
                R.id.action_listOfRoutesFragment_to_updateListOfRoutesFragment,
                bundleOf("key" to it.departureCountry)
            )
        }
