...
import com.leskov.airport.databinding.FragmentListOfAirplanesBinding
import com.leskov.airport.domain.entity.TypeOfEntity
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class ListOfAirplanesFragment :
    BaseVMFragment<ListOfAirplanesViewModel, FragmentListOfAirplanesBinding>() {

    override val layoutId: Int
        get() = R.layout.fragment_list_of_airplanes

    override val viewModel: ListOfAirplanesViewModel by viewModels()

    private val adapter = ListOfAirplanesAdapter()
    ...
    override fun initListeners() {
        ...
        binding.flAddItem.setOnClickWithDebounce {
            navController.navigate(
                R.id.action_listOfAirplanesFragment_to_insertNewItemFragment,
                bundleOf("type" to TypeOfEntity.AIRPLANE)
            )
        }

        adapter.setOnItemClickListener {
            navController.navigate(
                R.id.action_listOfAirplanesFragment_to_updateListOfAirplanesFragment,
                bundleOf("key" to it.model)
            )
        }
